...
from_bytes_to_human_readable() {
    assert_argument_provided "$1"

    local size_bytes="$1"
    local size_human_readable

    if [ "$size_bytes" -lt 1024 ]; then
        size_human_readable="${size_bytes}B"
    elif [ "$size_bytes" -lt $((1024 * 1024)) ]; then
        size_human_readable="$(convert_to_kb "$size_bytes")KiB"
    elif [ "$size_bytes" -lt $((1024 * 1024 * 1024)) ]; then
        size_human_readable="$(convert_to_mb "$size_bytes")MiB"
    else
        size_human_readable="$(convert_to_gb "$size_bytes")GiB"
    fi

    echo "${size_human_readable}"
}
