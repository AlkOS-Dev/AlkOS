...
feature_flags_generate_cxx_files() {
  ...
  for flag in "${flag_names[@]}"; do
    ...
    echo "    k${flag_upper}," >> "${FEATURE_FLAGS_CXX_PATH}"
  done
  ...
  for flag in "${flag_names[@]}"; do
    ...
    if [[ ${CONFIGURE_FEATURE_FLAGS[$flag]} == true ]]; then
      echo "template <> constexpr bool FeatureEnabled<FeatureFlag::k${flag_upper}> = true;" >> "${FEATURE_FLAGS_CXX_PATH}"
    else
      echo "template <> constexpr bool FeatureEnabled<FeatureFlag::k${flag_upper}> = false;" >> "${FEATURE_FLAGS_CXX_PATH}"
    fi
