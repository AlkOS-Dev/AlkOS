...
parse_args() {
    ...
    argparse_add_positional "shell" "Shell type" true "bash"
...
bash_enable_completion() {
  require_root_privileges

  # Check if bash-completion is installed
  if [[ ! -d /etc/bash_completion.d ]]; then
    dump_error "bash-completion is not installed. Please install it first."
  fi

  local script="$1"
  local script_name="$(basename "$script")"
  local target_link="/etc/bash_completion.d/alkos_completion.bash"
  ...
  if [[ -L "$target_link" ]]; then
    ...
    dump_error "A file named ${script_name} already exists in /etc/bash_completion.d/ and is not a symlink. Please remove it first."
  fi

  ln -s "$script" "$target_link"
  pretty_success "Created symlink for ${script_name} in /etc/bash_completion.d/. Please restart your terminal to take effect."
}

# Must be run with sudo
bash_disable_completion() {
  require_root_privileges
  ...
  if [[ -L "$target_link" ]]; then
    rm "$target_link"
    pretty_success "Removed symlink for ${script_name} from /etc/bash_completion.d/."
  elif [[ -e "$target_link" ]]; then
    dump_error "A file named ${script_name} exists in /etc/bash_completion.d/ and is not a symlink. Please remove it manually."
  else
    pretty_info "No symlink for ${script_name} found in /etc/bash_completion.d/. Exiting."
  fi
}
...
generate_bash_completion() {
...

  {
    ...
    for script in "${paths[@]}"; do
      ...
      get_definitions "$script"
      echo "            ;;"
    done
    echo "    esac"
    echo
    echo "    source \"${GENCOMP_DIR}/helper.bash\" # Lazy load"
...
main() {
    ...
    if [[ "$(argparse_get "r|run")" == "true" ]]; then
        ...
        case "$shell" in
            bash)
                generate_bash_completion "$output_path" "${GENCOMP_SCRIPTS_LIST[@]}"
                pretty_success "Bash completion script generated at: $(realpath completion.generated.bash)"
                ;;
            *)
                dump_error "Unsupported shell type: $shell. Currently, only 'bash' is supported."
                ;;
        esac
    fi
    ...
    if [[ "$(argparse_get "e|enable")" == "true" ]]; then
        ...
        case "$shell" in
            bash)
                prompt_to_execute true bash_enable_completion "$output_path"
                ;;
            *)
                dump_error "Unsupported shell type: $shell. Currently, only 'bash' is supported."
                ;;
        ...
        case "$shell" in
            bash)
                prompt_to_execute true bash_disable_completion "$output_path"
                ;;
            *)
                dump_error "Unsupported shell type: $shell. Currently, only 'bash' is supported."
                ;;
