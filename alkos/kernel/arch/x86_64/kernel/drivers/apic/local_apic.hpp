...
class LocalApic
{
     ...
     *
     * All registers are 32 bits wide but are aligned on 16-byte boundaries (4 bytes of data, 12
     * bytes reserved). Access type: RW = Read/Write, RO = Read Only, WO = Write Only
     *
     * Note: When accessing these registers, add the offset to the base MMIO address of the Local
     * APIC.
     */
    ...
    static constexpr u32 kSpuriousInterruptRegRW =
        0x0F0;  ///< [RW] Spurious Interrupt Vector Register - Global APIC enable and spurious
                ///< vector
