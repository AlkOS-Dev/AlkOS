...
SYSCALL_NAME(create_directory, kSysCreateDirectory, int, const char *, path);
SYSCALL_NAME(delete_file, kSysDeleteFile, int, const char *, path);
SYSCALL_NAME(move_file, kSysMoveFile, int, const char *, old_path, const char *, new_path);

/* Thread, processes */
SYSCALL_NAME(thread_create, kThreadCreate, int, Thread *, thread, thread_func_t, f, void *, arg);
SYSCALL_NAME(exec, kExec, u64, const char *, path, bool, async);
...
SYSCALL_NAME(get_heap_start, kGetHeapAddr, void *);
...
/* Input Syscalls */
SYSCALL_NAME(get_key_state, kSysGetKeyState, bool, VirtualKey, vk);
