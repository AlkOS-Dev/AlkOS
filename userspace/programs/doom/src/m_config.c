...
static void SaveDefaultCollection(default_collection_t *collection)
{
    ...
    for (i = 0; i < collection->numdefaults; i++) {
        ...
        for (; chars_written < 30; ++chars_written) fprintf(f, " ");
...
#else
        if (!strcmp(configdir, "/")) {
            // Special case: if configdir is just "/", avoid
            // "//doom/saves/"
            savegamedir = M_StringJoin(configdir, "doom/saves/", NULL);
        } else {
            savegamedir = M_StringJoin(configdir, "DIR_SEPARATOR_S", "doom/saves/", NULL);
        }
