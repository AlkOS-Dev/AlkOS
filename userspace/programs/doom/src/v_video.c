...
// accurate emulation.
//
void V_SetPatchClipCallback(vpatchclipfunc_t func) { patchclip_callback = func; }
...
void V_DrawRawScreen(byte *raw) { memcpy(dest_screen, raw, SCREENWIDTH * SCREENHEIGHT); }
...
static void warning_fn(png_structp p, png_const_charp s) { printf("libpng warning: %s\n", s); }
...
void WritePNGfile(char *filename, byte *data, int width, int height, byte *palette)
{
    ...
    png_set_IHDR(
        ppng, pinfo, width, height, 8, PNG_COLOR_TYPE_PALETTE, PNG_INTERLACE_NONE,
...
void V_ScreenShot(char *format)
{
    ...
    if (png_screenshots) {
        WritePNGfile(
            lbmname, I_VideoBuffer, SCREENWIDTH, SCREENHEIGHT,
...
void V_DrawMouseSpeedBox(int speed)
{
    ...
    if (linelen < redline_x) {
        ...
        V_DrawHorizLine(
            box_x + redline_x, box_y + MOUSE_SPEED_BOX_HEIGHT / 2, linelen - redline_x, yellow
        );
    }
