...
void AM_drawLineCharacter(
    mline_t *lineguy, int lineguylines, fixed_t scale, angle_t angle, int color, fixed_t x,
    fixed_t y
)
{
    int i;
...
void AM_drawPlayers(void)
{
    ...
    if (!netgame) {
        if (cheating)
            AM_drawLineCharacter(
                cheat_player_arrow, arrlen(cheat_player_arrow), 0, plr->mo->angle, WHITE,
                plr->mo->x, plr->mo->y
            );
        else
            AM_drawLineCharacter(
                player_arrow, arrlen(player_arrow), 0, plr->mo->angle, WHITE, plr->mo->x, plr->mo->y
            );
        return;
    }
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
        ...
        AM_drawLineCharacter(
            player_arrow, arrlen(player_arrow), 0, p->mo->angle, color, p->mo->x, p->mo->y
        );
    }
}
...
void AM_drawThings(int colors, int colorrange)
{
    ...
    for (i = 0; i < numsectors; i++) {
        ...
        while (t) {
            AM_drawLineCharacter(
                thintriangle_guy, arrlen(thintriangle_guy), 16 << FRACBITS, t->angle,
