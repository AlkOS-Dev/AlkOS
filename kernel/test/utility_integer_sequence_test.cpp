...
class IntegerSequenceTest : public TestGroupBase
{
};

TEST_F(IntegerSequenceTest, MakeIndexSequence_WithSizeN_GeneratesSequenceZeroToNMinusOne)
{
    using seq_0 = make_index_sequence<0>;
    using seq_1 = make_index_sequence<1>;
    using seq_5 = make_index_sequence<5>;

    static_assert(is_same_v<seq_0, index_sequence<>>);
    static_assert(is_same_v<seq_1, index_sequence<0>>);
    static_assert(is_same_v<seq_5, index_sequence<0, 1, 2, 3, 4>>);
...
TEST_F(IntegerSequenceTest, IntegerSequenceSize_ForGivenSequence_ReturnsCorrectCount)
{
